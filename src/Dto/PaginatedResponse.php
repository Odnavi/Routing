...
final class PaginatedResponse
{
     ...
     *
     * @param Collection   $collection Коллекция с посчитанным total.
     ...
     */
    public static function fromCollection(Collection $collection, string $itemDto, int $page, ?int $limit): self
    {
        $total = $collection->getTotal();

        return new self(
            items: $collection->toArray(
                static fn(AbstractEntity $entity): array => $itemDto::fromEntity($entity)->toArray()
            ),
            pagination: [
                'page'  => $page,
                'limit' => $limit,
